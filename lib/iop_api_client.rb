...
$logger = Logger.new($dir + 'iopsdk.log.' + Time.now.strftime('%Y-%m-%d'))
...
module IopApiClient
    ...
    class UrlConstants
        Api_gateway_url_tw = 'https://api-pre.aliexpress.com/sync'
        Api_authorization_url = 'https://auth.taobao.tw/rest'
    end
    ...
    class Client
        ...
        def execute(request,accessToken = nil)
            ...
            sys_params[:partner_id] = 'iop-sdk-ruby-20220609'
            sys_params[:simplify] = request.simplify
            sys_params[:format] = request.format
            ...
            if accessToken != nil
                sys_params[:session] = accessToken
            end

            if request.api_name .include? "/"
            ...
            return IopApiClient::Response.new(obj['type'],obj['code'],obj['message'],obj['request_id'],obj)
        end
